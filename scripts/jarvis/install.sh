...
ok "Automatic startup configured"
...
if [ -d "/data" ]; then
    ...
    AVAIL=$(df -h /data | tail -1 | awk '{print $4}')
    echo "  Disk: $USED used / $AVAIL available"
fi
...
# Test rápido
echo -n "  Jarvis test... "
RESPONSE=$(curl -s -X POST "http://localhost:8403/chat?message=Hello" 2>/dev/null)
if echo "$RESPONSE" | grep -q "response"; then
    echo -e "${GREEN}Jarvis replied!${NC}"
else
...
echo ""
echo -e "  ${YELLOW}════════════════════════════════════════════════${NC}"
echo -e "  ${YELLOW}  You can disconnect the internet now.${NC}"
echo -e "  ${YELLOW}  Jarvis will run 100% offline after setup.${NC}"
echo -e "  ${YELLOW}════════════════════════════════════════════════${NC}"
echo ""
echo "  Commands:"
echo "    Chat:     curl -X POST 'http://localhost:8403/chat?message=Hello'"
echo "    Memory:    curl http://localhost:8401/stats"
echo "    Emotions:  curl http://localhost:8401/emotions/current"
...
echo "    Conversation loop: sudo bash $JARVIS_DIR/scripts/jarvis/converse_loop.sh"
echo "    Uninstall: sudo bash $JARVIS_DIR/scripts/jarvis/uninstall.sh"
echo ""
echo -e "${CYAN}═══════════════════════════════════════════════════════${NC}"
